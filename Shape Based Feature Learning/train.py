...
import sys
sys.path.append('D:\Academics\Sem_2\Seminar\SFL_Ultimate\Ultimate\content\SFL_master')
...
def get_dataloader(root_dir, hospital: int,  split='train', batch_size=1, transform=None, shuffle=False, config=None,
                   num_workers=10):
...
def train(config_path, train_hospital: int, data_dir):
    ...
    dataloader = get_dataloader(data_dir, train_hospital, 'train', 4, transform, True, config.config)
    val_dataloader = get_dataloader(data_dir, train_hospital, 'val', 16, config=config.config)
